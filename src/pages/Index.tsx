    ...
    if (file) {
      ...
      setTimeout(() => {
        setResumeScore({
          ...
          buzzwords: 5,
          verifiability: 8,
          summary: 'A solid resume with good detail. Consider elaborating on one project and double-check grammar.'
        });
      }, 1200);
    }
  };
      ...
      <main className="max-w-5xl mx-auto p-4 sm:p-8 bg-[#181926] shadow-lg rounded-md space-y-10 mt-4 sm:mt-8 text-white">
        ...
        {mode === 'resume' && (
          <div className="border-2 border-dashed border-gray-700 rounded-lg p-8 text-center space-y-6 bg-[#23243a]">
            {!resumeFile && (
              <>
                <p className="mb-4 text-gray-700 text-lg">Upload your resume (PDF/DOCX) to get a free AI-powered review</p>
                <input
                  type="file"
                  accept=".pdf,.doc,.docx"
                  className="hidden"
            ...
            {resumeFile && resumeScore && (
              <div className="space-y-6">
                ...
                <ul className="space-y-1 text-gray-700">
                  <li>Grammar: <span className="font-medium">{resumeScore.grammar}/10</span></li>
                  <li>Technical: <span className="font-medium">{resumeScore.technical}/10</span></li>
                  <li>Consistency: <span className="font-medium">{resumeScore.consistency}/10</span></li>
                  <li>Buzzwords: <span className="font-medium">{resumeScore.buzzwords}/10</span> (lower is better)</li>
                  <li>Verifiability: <span className="font-medium">{resumeScore.verifiability}/10</span></li>
                </ul>
                <ScoreBreakdown data={resumeScore} />
                <div className="mt-4">
                  <p className="text-sm text-gray-500 italic">{resumeScore.summary}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="pt-8 text-center text-xs text-gray-500">
          Need help? <a href="https://github.com/yourrepo/hire-smart-navigator#readme" target="_blank" rel="noopener noreferrer" className="underline hover:text-blue-400">Read the docs</a>
        </div>
      </main>
    </div>
  );
